...
To denote potential outcomes in the survival setting, one would typically write $S_{T^x}(t)$, $F_{T^x}(t)$, or some variation thereof [@Hernan2005 (SNFT Paper)]. The important takeaway is that, once a potential outcome is written, it must be defined appropriately. In other words, one should always include a sentence such as: "where $S_{T^x}(t)$ is the survival function that would be observed for survival time $T$ had the individuals been exposed to $X = x$.
...
|               Please come up with a clearly articulated connection between correct model misspecification and exchangeability? Can you convince yourself beyond a doubt that these two things are related?
...
There are a number of different procedures one can use to evaluate whether positivity is a problem. Among these include propensity score overlap plots. Consider again our simple simulation example:
```{r, echo=T,fig.star=T,tidy=F,highlight=T}
