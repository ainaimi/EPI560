...
cohort %>% 
  ...
  mutate(ID=rank(time,ties="random"),
         ...
         outcome=if_else(ID==4,2,outcome)) %>% 
  ggplot(.) + 
...
The next sections will be about **how** to estimate the cumulative distribution function for a time-to-event outcome. We will be introduced to two different approaches, the Kaplan-Meier estimators, the cumulative incidence function estimator (from !^!Gray1988), and the Aalen-Johansen estimator [^!^Putter2007]. We'll also discuss the factors that should lead you to decide choosing one or the other, and go over how to use them in the R programming language. 

It's important to note here that the KM, CIF, and AJ estimators will estimate the same thing and yield the same results when there are no competing risks present. We will cover what competing risks are, and what happens to these estimators when competing risks are present in the next section.  
...
To implement the KM estimator in R, we need to use the `survival` package, which includes the `Surv()` and the `survfit()` functions. We will use the data in Table \@ref(tab:synthdat), and we have to set up the data in so the `Surv()` and `survfit()` functions work as we want them to.

The key issue we need to address is the following: in Table \@ref(tab:synthdat), we use the number "2" to denote Type I right censoring, and the number "0" to denote Type II right censoring. However, the functions in `R` do not distinguish between Type I and Type II censoring. We need set all these observation's (ID = 4, 8, 9, 10) outcome to the same value. We'll pick the number "0":
