...
p1 <- cens_dat %>% 
  ...
  scale_y_continuous(expand=c(0.05,0.05), 
                     breaks=seq(1,3,1),
                     limits=c(1,3)) +
  scale_x_continuous(expand=c(0,0.05),limits=c(-3,7)) +
  theme(axis.title.y = element_text(angle=0,vjust=.5)) +
...
ggsave(file="../figures/2021_11_23-figure2_cens_trunc.png",g)
...
```{r censtrunc, out.width="10cm", fig.fullwidth = TRUE, fig.cap="Six observations in a hypothetical study depicting censoring and truncation (left, right, and interval for both).",echo=F}
