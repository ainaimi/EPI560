...
To explore how we obtain estimates of the cause-specific risk function, let's use the cohort dataset (`section1_cohort.csv`). This dataset has 100 observations and six variables.
...
cohort <- cohort %>% mutate(cs_outcome = as.numeric(outcome==2))
...
The above code generates the `surv_model` object, creates a plot using this object with `ggplot`, and saves it to the `figures` folder. We can include the saved figure in our RMarkdown document using the following code:^[One could also use the generic `plot()` function to obtain the KM curve. For example: `plot(surv_model)`. However, it is much easier to modify the figure and adapt it to meet certain submission requirements using ggplot.]:

```{r cs_figure, out.width="10cm", fig.align='center', echo=T}
