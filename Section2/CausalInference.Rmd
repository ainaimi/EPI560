...
|               In a large body of methods literature, particularly those in econometrics, you are likely to encounter these causal assumptions articulated in different ways. Most commonly, researchers will often invoke **ignorability** as a core assumption in causal inference. There are at least two versions of ignorability: strong and weak ignorability. **Strong ignorability** is defined as the combination of the conditional independence assumption, and the positivity assumption. Technically, strong ignorability holds if, for individual $i$ with a binary exposure $X \in \{0,1\}$:
...
The distinctions between strong ignorabilty, weak ignorability, and mean exchangeability are of little practical consequence. While it is important to recognize that ignorability typically consistes of the combination of exchangeability and positivity. Additional details on and intuition behind the different versions of exchangeability can be found in Technical Point 2.1 of @Hernan2020.
...
A natural question that one may ask is whether there is a way we can conduct an analysis with observational data without having to worry at all about the "no measured or unmeasured confounding" assumption. It turns out, we can use instrumental variables to do this. Instrumental variables are sometimes presented as an "epidemiologist's dream" [@Hernan2006]. However, while in principle they do not require that we worry about measured or unmeasured confounding (which is great!), there are tradeoffs we make when we seek to analyse data using IVs [@Hernan2006]. 

## Randomization and ITT

To begin, let's revisit the randomized trial example from the section on identifying the ATE. Consider again the situation where we want to estimate the effect of aspirin on headache in a cohort of individuals aged 18-40 years. Again, we randomize 50% of the individuals in our sample to aspirin, and the other 50% to placebo. From this study, we can collect information on a number of things, beyond just the outcome of interest, and whether an individual was assigned to the treatment/placebo group. 
...
For example, we can again consider sleep quantity in the night previous. An individual who slept very little the night before may not feel up to taking their assigned treatment. Similarly, this same individual may be at a higher risk of experiencing headache. For this reason, we would collect information on sleep quantity in the night previous to randomization. 

The example given in this scenario allows us to draw a DAG summarizing the relations between the treatment assignment indicator, taking the treatment/placebo, confounder of taking and the outcome, and the outcome of interest. This DAG is shown in Figure \ref{fig:ivdag}:
...
$$ \psi = \frac{E(Y \mid X = 1) - E(Y \mid X = 0)}{E(A \mid X = 1) - E(A \mid X = 0)} $$
...
By definition, a variable is referred to as an "instrumental variable" if it is characterized by the following 

## Weak Instruments

## Homogeneity Makes the LATE --> ATE

## Monotonicity Makes the LATE --> CACE
